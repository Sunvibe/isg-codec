"""Capture large THZ 5.5 Eco aggregate requests through ser2net."""
...
def establish_request(sock: socket.socket, request_message: bytes, timeout: float, max_retry: int) -> bytes:
...
def receive_data(sock: socket.socket, timeout: float, max_retry: int) -> tuple[bytes, bytes]:
    response = bytearray()
    retries = 0
    sock.settimeout(timeout)
...
def capture_request(sock: socket.socket, request: BulkRequest, args: argparse.Namespace) -> tuple[bytes, bytes, bytes]:
    request_message = create_request_message(request.request_bytes)
    start_communication(sock, args.byte_timeout)
    data_available = establish_request(sock, request_message, args.byte_timeout, args.max_retry)
    sock.sendall(bytes([ESCAPE]))
    raw_response, response = receive_data(sock, args.byte_timeout, args.max_retry)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Capture large THZ 5.5 Eco aggregate requests through a ser2net endpoint.",
    )
    ...
    try:
        with socket.create_connection((args.host, args.port), timeout=args.connect_timeout) as sock:
            ...
            for index, request in enumerate(requests, start=1):
                ...
                try:
                    data_available, raw_response, response = capture_request(sock, request, args)
